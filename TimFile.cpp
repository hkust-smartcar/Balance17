		...
		CurTime = System::Time();
			...
			if (car_run && CurTime-PreTime >= 10) {
				...
				if (camEnable) {
					...
					if( (!ObstacleEnd) || Trackstate == Straight){//Trackstate == Crossroad || Trackstate == Crossroad2 || Trackstate == Crossroad3 ||
						limit = S_LIMIT;
						...
						if(curDiff > 6 || curDiff < -6 )
							differential = R_BP* (curDiff/30) + R_BD*(curDiff-prevDiff);
						else
							differential = R_P* (curDiff/30) + R_D*(curDiff-prevDiff);
						limit = R_LIMIT;
						if(differential > limit)
							differential = limit;
						else if(differential < -limit)
...
//					sprintf(buffer,"%.0f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.2f,%.2f\n",curSpeed,1.0, -1.0, 2.5, -2.5, 4.0, -4.0,curDiff,20*differential);
					sprintf(buffer,"\t%.0f\t%.2f\t%.2f\t%d\t%d\t%d\t%d\t%d\t%d\n",curSpeed,curDiff,differential,InRoundStart,InRoundEncoderValue,InRoundEnd,OutRoundStart,OutRoundEncoderValue,OutRoundEncoderValue);
